...
![Alt](/docs/img/System_components.png)
...
Зміст повідомлення повиннен бути наступним: "agr#PPPP#CCCC", де "agr" - номер агрегатора (поставщика послуг); "#PPPP" - номер паркомісця, на яке паркується клієнт; "#CCCC" - номер автомобіля, який паркується. 
СМС-центром для обробки таких повідомлень є спеціальний агрегатор, який виконує роль посередника між оператором і сервісом. Процес оплати смс відбуваєтсья наступним чином: 
1. Відправка смс з текстом типу "agr#PPPP#CCCC".
2. Після отримання смс від клієнта агрегатор відправляє запит POST на вказану URL-адерсу сервісу з наступними параметрами: 
- ***sms_id*** - унікальний ідентифікатор смс повідомлення;
- ***sms_body*** - текст повідомлення абонента;
- **site_service_id*** - id послуги агрегатора;
- ***user_num*** - номер телефону абонента;
- ***cpref*** - додатковий префікс;
- ***sms_price*** - вартість смс разом з ПДВ;
- ***sms_currency*** - Валюта, згідно стандарту ISO-4217. Наприклад, USD, UAH, т.д.;
- ***partner_cost*** - дохід партнера (у даному випадку сервісу).
- ***partner_currency*** - валюта доходу;
- ***secret_key*** - контрольне значення, вирахуване на основі основних параметрів, а також значения secret_word. Служить для перевірки валідности запиту. Формула обчислення контрольного значення: secret_key = md5(sms_id + sms_body + site_service_id + operator_id + num + sms_price + 'Секретне слово'). 
3. Протягом 30 секунд сервіс повинен дати відповідь у наступному форматі: 
- sms_id:уникальний ідентифікатор смс повідомлення "\n";
- response: відповідь партнера в форматі UTF-8 "\n";
- error: 0 або  1; 
у випадку відсутності відповіді від сервісу, агрегатор надсилає клієнтові повідомлення про неможливість надання послуги. 
4. У випадку проходження успішної оплати агрегатор інформує сервіс запитом POST результат операції у форматі: 
- sms_id - унікальний ідентифікатор смс;
- status - результат оплати смс абонентом. Приймає значення 1, якщо повідомлення доставлено і оплачено, або ж 0 - якщо не оплачене або ж не доставлене;
- user_num - номер телефону абонента;
- site_service_id - id послуги агрегатора
5. Сервер сервісу завершує транзакцію оплати за паркування.  
6. У відповідь клієнтові приходить смс з підтвердженням. 
...
#Користувачі. Взаємодія користувача з системою. 
## Види користувачів

Коритувачів (клієнтів) сервісу можна розділити на: 
- ***не зареєстровані***: мають можливість: 
	* оплачувати парковку;
	* переглядати цінову політику парковок;
	* робити пошук за ціною;
- ***зареєстровані***: перевагою над незареєстрованими є відсутність необхідності вводити номер машини кожного разу завдяки автоматичному запамятовуванню у профілю користувача. Також існує можливість зберегти попередні реквізит оплати для пришвидшення подальшої оплати (збереження номеру рахунку і типу карти);

Типовими сценаріями використання сервісу клієнтами є: 
...
* Як користувач, у разі помилки я хочу бачити її причини і методи вирішення

Сервісні користувачі системи поділяються на наступні типи: 
- інспектори - здійснюють контроль за оплатою паркування безпосередньо на парковці, виписують штрафи при відсутності оплати за парковку. Повинні мати можливість у системі:
	* перевіряти список оплачених автомобілів на конкретній парковці з вказанням терміну стоянки. 
	* Поточна парковка для перевірки повинна підгружатися автоматично залежно від місця перебування інспектора.   
- адміністратори - користувачі, що здійснюють контроль над роботою системи. Мають змогу: 
	* переглядати статистику використання (навантаженості) парковки за певним діапазоном дат;
	* переглядати статистику використання різних видів оплат за різні часові проміжки;
	* мати можливість модифікувати цінову політику парковки;
	* переглянути завантаженість кіосків готівкою та розхідними матеріалами;

Типовими сценаріями вимог сервісних користувачів до системи є:
...
## Алгоритми взаємодії клієнтів з системою.

***Оплата через інтернет***
Дії:
1) Перейти по адресі Park.lviv.ua
<br>![Alt](/docs/img/111.jpg)<br>
Користувач опиниться на початковій сторінці аплікації, де одразу можна здійснити оплату. Тут присутні поля для вводу та кнопка переходу до наступного кроку;

2) В вказані поля необхідно ввести номер паркомісця (його користувач бачить безпосередньо на місці паркування), номер машини;

3) Після цього користувачеві представляються активні тарифи та можливість ввести час в годинах, скільки він буде стояти;
<br>![](/docs/img/222.jpg)<br>
...
<br>![Alt](/docs/img/444.jpg)<br>

***Оплата через СМС***
Дії:
1) Поставивши машину, користувач на інформативній дошці бачить шаблон смс-повідомлення ("#Parking1 #ВС1111ВС") для оплати парковки;
2) Відправляє смс на один з запропонованих номерів, який визначає за скільки часу буде здійснюватися оплата;
3) У відповідь отримує підтвердження про оплату у вигляді: #car_number на #parking_nubber може стояти до ##год;
<br>![Alt](/docs/img/mob.jpg)<br>

***Оплата через кіоск***
Дії:
1) На екрані демонструються тарифи парковки та форма для оплати.
<br>![Alt](/docs/img/parkom.png)<br>
...
1) Loсation API (наприклад Google![Alt](/docs/img/333.jpg)<br>
 Maps Geolocation API, Geoloqi API чи інші) визначає місцезнаходження авторизованого інспектора.
...
1) Для користувача дають: 

* наочне представлення розміщення парковок
* можливість прокладання маршруту до заданої парковки від певної точки

2) При виборі конкретної парковки

* показують ємність парковки
* поточну завантаженість (кількість вільних місць)
* можливість перейти до режиму її оплати(бронювання) 
...
#Безпека з'єднання кіоску з сервером
Виходячи з того, що більшість кіосків - це комп'ютери, з "вічно" відкритими браузерами і обмеженими користувацькими функціями, ми будуємо з'єднання від кіоску з сервером на основі HTTPS. При інсталяції кіоску, в його браузер завантажується сертифікат серверу CentralPark, що дасть до нього доступ. Це униможливить атаки типу man-in-the-middle. Ми будемо використовувати протокол OpenSSL, адже він є бесплатним і не потребує з'єднань з сертифікаційним авторизованим центром.